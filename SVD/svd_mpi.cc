/**
Computes SVD for tall and skinny matrices ( rows >>> cols) using MPI (QR
decomposition method) the final matrices U would be distributed over all nodes,
Matrices V and sigma would be avaiable on root (rank == 0)

Orthogonality checks can be done by computing L2 norm - |(U^T * U) - I_n |_2 of
the resultant matrices to check the correcness of the results

To see sample output matrices in stdout or in file, simply do cout<<v; or
fout<<v;
*/
...
int main(int argc, char *argv[]) {
  ...
  MPI::COMM_WORLD.Scatter(data, N * cols, MPI::DOUBLE, recvbuf, N * cols,
                          MPI::DOUBLE, 0);
  ...
  // task parallelism - matrix q2 and jacobi svd for small matrices being
  // calculated simultaneously
  if (myrank == 0) {
    RMatrixXd s = Map<RMatrixXd>(stack, npes * cols, cols);
    LLT<RMatrixXd> lltOfS(s.transpose() * s);
    res2 = lltOfS.matrixL()
               .transpose(); // fast - works on positive definite matrix
    // cout << "res : " << res << endl;
